...
$record = Join-Path $repo 'artifacts/shopify-tunnel-processes.json'
...
foreach ($entry in $owned.PSObject.Properties) {
    ...
    if (!$process) { continue }
    if (($entry.Name -eq 'proxy' -and $process.CommandLine.Contains((Join-Path $PSScriptRoot 'shopify-tunnel-proxy.cjs'))) -or
        ($entry.Name -eq 'tunnel' -and $process.ExecutablePath -eq (Join-Path $repo 'artifacts/cloudflared.exe') -and $process.CommandLine.Contains('http://127.0.0.1:5523'))) {
