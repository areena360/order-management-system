// Loopback-only gateway for temporary WooCommerce authorization. No admin CRUD/files.
...
http.createServer((req,res) => {
  ...
  if (allowed) {
    ...
    const headers = {...req.headers, host:'localhost:52984'};
    for (const h of ['forwarded','x-forwarded-host','x-forwarded-proto','x-forwarded-for','connection']) delete headers[h];
    const upstream = http.request({host:'localhost',port:52984,path:req.url,method:req.method,headers}, response => {
...
}).listen(5522,'127.0.0.1',()=>console.log('WooCommerce gateway listening on 127.0.0.1:5522'));
